...
import { getAvailableWhatsAppTemplates, renderEmailTemplate } from '../utils/templates';
import { BulkActionBar } from './table/BulkActionBar';
import { TableFilters } from './table/TableFilters';
import { CompareDuplicateModal } from './CompareDuplicateModal';
...
export const LeadsCRMTable: React.FC<LeadsCRMTableProps> = ({
  ...
  const [compareDuplicateTarget, setCompareDuplicateTarget] = useState<{ original: Lead, duplicate: Lead } | null>(null);
  ...
  return (
    <div className="space-y-4">
      ...
      <TableFilters
        ...
        quickChipFilter={quickChipFilter} setQuickChipFilter={setQuickChipFilter}
        opportunityFilter={opportunityFilter} setOpportunityFilter={setOpportunityFilter}
        contactInfoFilter={contactInfoFilter} setContactInfoFilter={setContactInfoFilter}
        categoryFilter={categoryFilter} setCategoryFilter={setCategoryFilter}
        handleCopyTSV={handleCopyTSV} copiedTSV={copiedTSV}
        handleExportCSV={handleExportCSV} setIsAddLeadModalOpen={setIsAddLeadModalOpen}
      />
      ...
      <BulkActionBar 
        ...
        onBatchMoveToApproved={handleBatchMoveToApproved}
        onBatchApplyCampaign={handleBatchApplyCampaign}
        onRunBatchAIAudit={() => onRunBatchAIAudit(Array.from(selectedLeadIds))}
        onOpenBatchNoteModal={() => setIsBatchNoteModalOpen(true)}
                          ...
                          <div className="flex flex-wrap gap-1 mt-1 items-center">
                            {lead.customTags.slice(0, 2).map((tg, idx) => (
                              <React.Fragment key={idx}>
                                <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${tg.startsWith('Duplicate of:') ? 'bg-rose-100 text-rose-700 border border-rose-200' : 'bg-slate-100 text-slate-600'}`}>
                                  {tg}
                                </span>
                                {tg.startsWith('Duplicate of:') && lead.duplicateOfId && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      const original = leads.find(l => l.id === lead.duplicateOfId);
                                      if (original) {
                                        setCompareDuplicateTarget({ original, duplicate: lead });
                                      } else {
                                        alert('Original lead not found.');
                                      }
                                    }}
                                    className="text-[9px] px-1.5 py-0.5 rounded font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 hover:bg-indigo-100 transition-colors"
                                  >
                                    Compare
                                  </button>
                                )}
                          ...
                          <div className="text-[11px] text-slate-600 flex items-center space-x-2 mt-1 font-medium">
                            ...
                            <button 
                              onClick={() => {
                                ...
                                const waTemplates = getAvailableWhatsAppTemplates(settings);
                                const currentTemplate = waTemplates[0]; // Use default WhatsApp template
                                const rendered = renderEmailTemplate(currentTemplate, lead, settings);
                                const text = encodeURIComponent(rendered.body);
      ...
      {compareDuplicateTarget && (
        <CompareDuplicateModal
          originalLead={compareDuplicateTarget.original}
          duplicateLead={compareDuplicateTarget.duplicate}
          onClose={() => setCompareDuplicateTarget(null)}
          onMerge={(mergedLead, duplicateId) => {
            onUpdateLead(mergedLead);
            onDeleteLead(duplicateId, true);
            setCompareDuplicateTarget(null);
          }}
          onDeleteDuplicate={(duplicateId) => {
            onDeleteLead(duplicateId, true);
            setCompareDuplicateTarget(null);
          }}
        />
