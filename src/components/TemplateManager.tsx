...
export const TemplateManager: React.FC<TemplateManagerProps> = ({ customTemplates, onChange }) => {
  ...
  const [editingDraft, setEditingDraft] = useState<EmailTemplate | null>(null);
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [copiedTag, setCopiedTag] = useState<string | null>(null);
  const bodyRef = useRef<HTMLTextAreaElement>(null);

  const editingTemplate = editingDraft;
  ...
  const handleDelete = (id: string) => {
    ...
    if (confirm('Delete this template? This cannot be undone.')) {
      ...
      if (editingId === id) { setEditingId(null); setEditingDraft(null); }
  ...
  const insertVariable = (tag: string) => {
    if (!editingDraft || !bodyRef.current) return;
    const textarea = bodyRef.current;
    const start = textarea.selectionStart;
    const end = textarea.selectionEnd;
    const before = editingDraft.body.substring(0, start);
    const after = editingDraft.body.substring(end);
    setEditingDraft({ ...editingDraft, body: before + tag + after });
    setTimeout(() => {
      textarea.focus();
                  ...
                  <div className="px-4 pb-4 border-t border-indigo-200 pt-3 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={e => setEditingDraft({ ...editingTemplate, name: e.target.value })}
                      ...
                      <div className="space-y-1">
                        ...
                        <select
                          ...
                          onChange={e => setEditingDraft({ ...editingTemplate, category: e.target.value })}
                          className="w-full bg-white border border-slate-200 rounded-lg p-2 text-xs font-medium focus:ring-2 focus:ring-indigo-500 outline-none"
                        >
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={e => setEditingDraft({ ...editingTemplate, subject: e.target.value })}
                        className="w-full bg-white border border-slate-200 rounded-lg p-2 text-xs font-medium focus:ring-2 focus:ring-indigo-500 outline-none"
                    ...
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        onChange={e => setEditingDraft({ ...editingTemplate, body: e.target.value })}
                    ...
                    <div className="flex justify-between items-center pt-2">
                      <span className="text-[10px] text-slate-400">{editingTemplate.body.length} chars</span>
                      <div className="flex space-x-2">
                        <button type="button" onClick={handleCancel} className="px-3 py-1.5 text-xs font-bold text-slate-500 hover:text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors flex items-center space-x-1">
                          <X className="w-3 h-3" />
                          <span>Cancel</span>
                        </button>
                        <button type="button" onClick={handleSave} className="px-4 py-1.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg flex items-center space-x-1 shadow-sm transition-colors">
